...
class TargetedLafeatAttack():
    ...
    def attack_single_run(self, x_in, y_in):
        ...
        for _ in range(self.eot_iter):
            with torch.enable_grad():
                ...
                scale_17 = self.scale_17 * (1 - 0 / self.n_iter)
                ##############################################
        ...
        step_size_original = self.eps * torch.ones([x.shape[0], 1, 1, 1]).to(self.device).detach() * torch.Tensor(
            [2.0]).to(self.device).detach().reshape([1, 1, 1, 1])
        x_adv_old = x_adv.clone()

        for i in range(self.n_iter):
            if self.linear:
                step_size = step_size_original * (1 - i / self.n_iter)
    ...
    def perturb(self, x_in, y_in, best_loss=False, scale_16=0, scale_17=0):
